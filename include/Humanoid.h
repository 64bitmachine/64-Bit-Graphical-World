...
enum partName 
{
	Head, Neck, Torso, LeftArm,
	LeftHand, RightArm, RightHand,
	LeftThigh, LeftLeg, RightThigh, RightLeg, Plate
};
...
public:
	...
	GLint ShaderProgram;
	BodyParts(GLfloat a, GLfloat b, GLfloat c, glm::vec3 Center, GLint d, partName h, GLuint k);
	void drawBodyPart();
	...
	void swingHand(GLint a)
	{
		...
		else if (this->type == LeftArm)
		{
			if (a == 1) 
			{
				if (this->angle1 < 65.0f) this->angle1 += 5.0f;
				// else if (this->angle1 < 0.0f) this->angle1 = 0.0f;
			}
			else if (a == 2) 
			{
				if (this->angle2 < 65.0f) this->angle2 += 5.0f;
...
public:
	...
	Humanoid(glm::vec3 center, GLint ShaderProgram);
	void addBodyPart(BodyParts* part, glm::vec3 Offset);
	void displayHumanoid();
	void rotCen()
	{
		root->cen += 5.0f;
	}
